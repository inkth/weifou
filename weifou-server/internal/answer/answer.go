...
// validStages 是 stage 字段允许的访客意向阶段枚举。
var validStages = map[string]bool{"chat": true, "ask": true, "connect": true}
...
type Result struct {
	...
	Options     []string `json:"options"`     // 下一步的可点选项（访客口吻、点选即发）
	Stage       string   `json:"stage"`       // 当前意向阶段：chat|ask|connect
}
...
// guidedOutputSpec 是访客对话（chat）的输出契约：分身按了解→深入→联系生成可点选项。
...
stage：本轮对话所处的意向阶段，取以下之一：
  chat（初识·了解 TA）/ ask（深入·聊到具体问题）/ connect（访客明确想联系本人、合作或交换名片）。
  自然推进：开场给了解型选项并标 chat；聊出具体问题后给深入型选项并标 ask；访客明确表达进一步沟通意愿时才标 connect。
  务必克制：不要一上来索要联系方式，不要引导任何付款行为。`

// plainOutputSpec 是问答箱（qabox 单发）的输出契约：只需回答 + 追问候选，无意向阶段。
...
// guided=true 走访客对话的引导契约（options+stage）；false 走问答箱的追问契约（suggestions）。
